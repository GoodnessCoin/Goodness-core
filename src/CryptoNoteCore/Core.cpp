...
std::error_code Core::addBlock(const CachedBlock& cachedBlock, RawBlock&& rawBlock) {
  ...
  if (previousBlockIndex <= 6357) rewardBlocksWindow = 50; // Goodness historical quirk
  ...
  if (minerReward != reward && previousBlockIndex >= CryptoNote::parameters::IRIDIUM_REWARD_ADJUSTMENT_BLOCK) { // permissive for Goodness's history
